...
empsal_df['conv'] = empsal_df.salary * .10
empsal_df['total'] = empsal_df.salary + empsal_df.hra + empsal_df.conv
print(empsal_df.head())

# Find the total of Salary, Hra, conv and total - state wise
empsal_df['conv'] = empsal_df.salary * .10
empsal_df['total'] = empsal_df.salary + empsal_df.hra + empsal_df.conv
sum_list = empsal_df.groupby(['state'])['salary', 'hra', 'conv', 'total'].sum()
print(sum_list)

# Plotting a bar chart of total figures of salary, hra, conv, and total
